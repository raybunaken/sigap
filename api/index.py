...
3. Judge each item in "items_to_judge": status met/partial/missing based on CV evidence.
   ...
   - EXPERIENCE rules: judge RELEVANCE to this job's domain, not total years worked. Unrelated domains (e.g. nurse vs data analyst) = MISSING. Adjacent domains (e.g. business analyst vs data analyst) = PARTIAL. SAME domain but years fall short (e.g. 6 months as Sales Operations Analyst vs "2+ years in Sales Operations") = PARTIAL, NEVER MISSING - state in the detail that the domain matches and only the duration is short.
...
5. "requirements_check": Array of 5-7 MOST IMPORTANT HARD REQUIREMENTS.
  ...
  Example you MUST follow: "Proficiency in Microsoft Excel, including Pivot Tables, XLOOKUP, Power Query" -> must_skills: ["Microsoft Excel"]; plus_skills: ["Pivot Tables", "XLOOKUP", "Power Query"]. Never list Pivot Tables, XLOOKUP or Power Query as separate must_skills - they are features of Excel.
